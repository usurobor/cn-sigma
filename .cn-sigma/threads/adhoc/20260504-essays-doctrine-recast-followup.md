...
The activation essays were drafted from product/architecture intuition and only later cited the agent doctrine ([CFA, EFA, JFA, IFA](https://github.com/usurobor/cnos/blob/main/docs/concepts/doctrine/README.md)) as ground. The current branch ships with a `Doctrine lineage` section in each essay that names the connection but does not derive the architecture from it.
...
For [`docs/papers/AGENT-CONTINUITY.md`](https://github.com/usurobor/cnos/blob/main/docs/papers/AGENT-CONTINUITY.md) and [`docs/papers/ACTIVATION-NOT-DEPLOYMENT.md`](https://github.com/usurobor/cnos/blob/main/docs/papers/ACTIVATION-NOT-DEPLOYMENT.md):
...
- It does not change the placement: essays stay in `docs/papers/`, doctrine stays in `docs/concepts/doctrine/`.
- It does not require reopening IFA's closure stance.

---

*Migrated from `cnos:threads/adhoc/20260504-essays-doctrine-recast-followup.md`
(present through `cnos@d227273b`; removed in `cnos@5991ea26`) per directive-out
`cnos@e4b61cf8` (2026-06-29T14:06Z). Paths repointed:
`docs/alpha/essays/` → `docs/papers/`, `docs/alpha/doctrine/` → `docs/concepts/doctrine/`.*
