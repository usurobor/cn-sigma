...
| `.cdd` dematerialization (ancestry plane) | cnos #682 + **design PR #689** | L7 design doc. External-β returned **REQUEST CHANGES** (D1–D12, C1–C4); **R1 pushed** — reworked into a typed seal-event protocol (`cn.cdd.seal.v1` + trailers + `refs/cn/cdd/seals/*` locator), four separated authorities (INDEX/CURRENT/#683-radar/live), CHAIN vs CONTENT DAGs, rejected-episode Git algo, prune predicate + dry-run, retention/trust, acquisition contract, per-episode fragments. **Status Proposed; ratification path B (independent CDS cell) recommended; prune NOT authorized; impl NOT derived until ratified** |
| ↳ M1 interim relief (from the review) | cnos dispatch/CI/search | **shippable now, independent of #682:** extend the existing sparse-checkout `.cn-sigma` exclusion to closed `.cdd` payloads — removes most agent-attention cost immediately while M4 (#682) is proven/ratified. Not yet filed |
...
| Activation protocol — five-case taxonomy + no-confabulated state **and identity** | cnos #686 → **PR #687** | #686 authored via `cdd/issue` (five cases: repo-less · home · attached-foreign · unattached-attachable · present-not-an-attach-target; five binding invariants; three named checks). WC implemented `agent/activate` + `agent/attach`; **δ-accepted** (base-parity verified, matter checked vs diff); **PR #687 open, awaiting operator merge**. #685 closed as superseded (folded in) |
| **PERSONA.md identity correction** (dualist core-drive → non-dualist; drop "Intelligent Assistants Team" + operator-coupling; role = OSS engineer who shipped cnos + others) | cn-sigma `spec/PERSONA.md` | **proposed, HELD for operator approval** (constitutive file — no-auto-apply gate). Diff staged; awaiting go |
