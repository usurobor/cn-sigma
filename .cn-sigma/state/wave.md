...
| Activation bootstrap modes (4 cases) + no-confabulated state **and identity** | cnos #685 (+ refinement comment 5150423513) | design-first. Repo-less Sigma recited stale state AND confabulated identity (dualist core-drive, "assistant team" / operator-coupling). Four cases: in-repo→id the repo · attached→report state+next-step · not-attached→ask + explain repo-side changes · no-repo→introduce + ask. Quiet activation; no state/doctrine dump |
