...
| `.cdd` dematerialization (ancestry plane) | cnos #682 + **design PR #689** | L7 design doc written (`docs/architecture/CDD-DEMATERIALIZATION.md`): main = "what is"; closed receipts → ancestry; HEAD keeps only a CHANGELOG-scale `.cdd/INDEX/{year}.jsonl` + `CURRENT.json`. Concrete current(1022 files/10.4MB)→target→S≺D≺P→4-phase migration. **Design PR open; impl NOT dispatched — awaits operator go** |
